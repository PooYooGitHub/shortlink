...
import com.project.dto.req.ShortLinkGroupStatsAccessRecordReqDTO;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO groupShortLinkStats(ShortLinkGroupStatsReqDTO requestParam) {
        ...
        rangeDates.forEach(each -> listStatsByGroup.stream()
                ...
                .ifPresentOrElse(item -> {
                    ShortLinkStatsAccessDailyRespDTO accessDailyRespDTO = ShortLinkStatsAccessDailyRespDTO.builder()
                            .date(DateUtil.parse(each,"yyyy-MM-dd"))
                            .pv(item.getPv())
                            .uv(item.getUv())
                            .uip(item.getUip())
                            .build();
        ...
        // 地区访问详情（仅国内）
        List<ShortLinkStatsLocateCNRespDTO> localeCnStats = new ArrayList<>();
        List<LinkLocateStatsDO> listedLocaleByGroup = linkLocateStatsMapper.listLocaleByGroup(requestParam);
    ...
    public IPage<ShortLinkStatsAccessRecordRespDTO> shortLinkStatsAccessRecord(ShortLinkStatsAccessRecordReqDTO requestParam) {
        ...
        LambdaQueryWrapper<LinkAccessLogsDO> eq = Wrappers.lambdaQuery(LinkAccessLogsDO.class)
                .eq(LinkAccessLogsDO::getGid, requestParam.getGid())
                .eq(LinkAccessLogsDO::getFullShortUrl, requestParam.getFullShortUrl())
                .eq(LinkAccessLogsDO::getDelFlag, 0)
                .between(LinkAccessLogsDO::getCreateTime, requestParam.getStartDate(), requestParam.getEndDate())
                .orderByDesc(LinkAccessLogsDO::getCreateTime);
        IPage<LinkAccessLogsDO> linkAccessLogsDOPage = linkAccessLogsMapper.selectPage(requestParam, eq);
